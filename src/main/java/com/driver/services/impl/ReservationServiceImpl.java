    ...
    public Reservation reserveSpot(Integer userId, Integer parkingLotId, Integer timeInHours, Integer numberOfWheels) throws Exception {
        ...
        try {
            ...
            if(!userRepository3.findById(userId).isPresent() || !parkingLotRepository3.findById(parkingLotId).isPresent()) {
                throw new Exception("Reservation cannot be made as either User or the Parking Lot not found");
            }
            ...
            assert choosenSpot != null;
            choosenSpot.setOccupied(true);

            Reservation reservation = new Reservation();
            reservation.setNumberOfHours(timeInHours);
            reservation.setSpot(choosenSpot);
            reservation.setUser(user);

            //Bidirectional mapping
            choosenSpot.getReservationList().add(reservation);

            user.getReservationList().add(reservation);

            userRepository3.save(user);
            spotRepository3.save(choosenSpot);
